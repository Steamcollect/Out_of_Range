...
namespace FischlWorks_FogWar
{
    ...
    public class Shadowcaster
    {
        ...
        public class FogField
        {
            ...
            public void Reset(FogWarManager fogWarManager)
            {
                foreach (LevelColumn levelColumn in levelRow)
                {
                    levelColumn.Reset(fogWarManager);
                }
            }

            public Color[] GetColors(float fogPlaneAlpha, FogWarManager fogWarManager)
            {
                if (colors == null)
                {
                ...
                for (int xIterator = 0; xIterator < levelRow[0].Count(); xIterator++)
                {
                    for (int yIterator = 0; yIterator < levelRow.Count; yIterator++)
                    {
                        ...
                        if (fogWarManager.keepRevealedTiles == true && visibility == (int)LevelColumn.ETileVisibility.PreviouslyRevealed)
                        {
                            tileOpacity = fogWarManager.revealedTileOpacity;
                        }
        ...
        public class LevelColumn
        {
            ...
            public void Reset(FogWarManager fogWarManager)
            {
                ...
                {
                    if (fogWarManager.keepRevealedTiles == false)
                    {
        ...
        {
            public QuadrantIterator(FogWarManager fogWarManager)
            {
                this.m_FogWarManager = fogWarManager;
            }
            ...
            public Vector2Int QuadrantToLevel(Vector2Int quadrantVector)
            {
                ...
                return m_FogWarManager.WorldToLevel(m_FogWarManager.GetWorldVector(quadrantPoint));
            }
            ...
            private FogWarManager m_FogWarManager = null;
        }
        ...
        public void Initialize(FogWarManager fogWarManager)
        {
            this.m_FogWarManager = fogWarManager;

            for (int xIterator = 0; xIterator < fogWarManager.levelData.levelDimensionX; xIterator++)
            {
                // Adding a new list for column (y axis) for each unit in row (x axis)
                fogField.AddColumn(new LevelColumn(
                    Enumerable.Repeat(LevelColumn.ETileVisibility.Hidden, fogWarManager.levelData.levelDimensionY)));
            }

            quadrantIterator = new QuadrantIterator(fogWarManager);
        }
        ...
        {
            fogField.Reset(m_FogWarManager);
        }
        ...
        public void ProcessLevelData(Vector2Int revealerPoint, int sightRange)
        {
            ...
            RevealTile(m_FogWarManager.WorldToLevel(m_FogWarManager.GetWorldVector(revealerPoint)));
        ...
        private void RevealTileIteratively(Vector2Int quadrantPoint, int sightRange)
        {
            ...
            if (m_FogWarManager.CheckLevelGridRange(levelCoordinates) == false)
            {
                return;
            }
        ...
        private bool IsTileEmpty(Vector2Int quadrantPoint)
        {
            ...
            return (m_FogWarManager.levelData[levelCoordinates.x][levelCoordinates.y] == FogWarManager.LevelColumn.ETileState.Empty);
        }
        ...
        private bool IsTileObstacle(Vector2Int quadrantPoint)
        {
            ...
            return (m_FogWarManager.levelData[levelCoordinates.x][levelCoordinates.y] == FogWarManager.LevelColumn.ETileState.Obstacle);
        }
